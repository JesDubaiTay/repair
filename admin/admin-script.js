...
async function loadLiveOrders() {
  ...
  if (!tableBody) return;
  tableBody.innerHTML = `<tr><td colspan="9" style="text-align:center; padding: 20px;">Загрузка заявок из базы...</td></tr>`;
  ...
  try {
    ...
    if (orders.length === 0) {
      tableBody.innerHTML = `<tr><td colspan="9" style="text-align:center; padding: 20px;">Новых заявок пока нет.</td></tr>`;
      return;
    }
    ...
    orders.forEach(order => {
      ...
      row.innerHTML = `
        ...
        <td>${order.device_type || 'Не указано'}</td>
